...
module.exports = function(args) {
  ...
  var noDeps = !cfg.deps || args.indexOf('--no-deps') > -1

  // remove this option from argument list
  if (noDeps) {
    args.splice(args.indexOf('--no-deps'), 1)
  }
  ...
  watcher.on('fallback', function(limit) {
    ...
    log.info('or use `--no-deps` option to consume less file descriptors.')
  })
  ...
  function start() {
    ...
    ipc.on(child, 'required', function(m) {
      if (noDeps && isInDifferentPackage(currentDir, m.required)) {
        return
      }
